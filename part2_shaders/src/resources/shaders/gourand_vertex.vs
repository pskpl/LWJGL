...
layout (location=2) in vec3 textureNormal;
...
vec4 calcLightColor(vec3 lightColour, float lightIntensity, vec3 verPos, verNormal, vec3 toLightDir)
{
	vec4(0, 0, 0, 0);
	...
	specularColor = vec4(lightColor, 1) * lightIntensity * specularFactor * material.refelectance;

	return diffuseColor + specularColor;
}

vec4 calcDirectionalLight(directionalLight, verPos, verNormal)
{
